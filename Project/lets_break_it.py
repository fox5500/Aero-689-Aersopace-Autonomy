...
def generate_bayes_nets(nrows,ncols,storm_prob,sandy_prob):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            ppm_cpt = {
                ('Adjacent', True): [0.95,0.04,0.01],
                ('Adjacent', False): [0.9,0.09,0.01],
                ('Diagonal', True): [0.3,0.5,0.2],
                ('Diagonal', False): [0.25,0.5,0.25],
                ('NotNearby', True): [0.01,0.24,0.75],
                ('NotNearby', False): [0.01,0.09,0.9]
            }
...
def compute_storm_probs(bayes_nets, ppm_evidence, sandyground_evidence, facts, grid):
    ...
    # Combine ppm and sandyground evidence
    evidence_dict = {}
    for key in ppm_evidence:
        evidence_dict[key] = {'PPM': ppm_evidence[key]}
    for key in sandyground_evidence:
        if key in evidence_dict:
            evidence_dict[key]['sandy'] = sandyground_evidence[key]
        else:
            evidence_dict[key] = {'sandy': sandyground_evidence[key]}
                    ...
                    for dj in [-1, 0, 1]:
                        ...
                        if 0 <= adj_i < 6 and 0 <= adj_j < 6:
                            if cell_properties.get((adj_i, adj_j), {}).get('safe') != 'true':
                                neighbor_evidence = evidence_dict.get((adj_i, adj_j), {})
                                if isinstance(neighbor_evidence, dict):
                                    combined_evidence.update(neighbor_evidence)
...
def get_fault_state(last_fault_state):
    ...
    if wheelFailure:
        ...
        if random.uniform(0,1) < 0.1:
            fault_info.append("MovementDrifts")
    #print(fault_info, unresolved_fault_state, 'fault_info, unresolved') ##DEBUG
...
def get_fault_prediction(fault_info, fault_bn):
    ...
    #print(prediction_dict, 'rediction') ##DEBUG
    ### YOUR CODE HERE ### 

    return prediction_dict

def get_fault_reaction(fault_prediction):
    # Define thresholds for reacting to faults
    reaction_thresholds = {
        "BatteryFault": 0.09,        # Threshold for reacting to BatteryFault
        "SolarPanelDusty": 0.4,     # Threshold for reacting to SolarPanelDusty
        "RockInWheel": 0.09         # Threshold for reacting to RockInWheel
    }

    # Initialize the fault reaction list
    fault_reaction = []

    # Iterate through each fault and check if it exceeds the reaction threshold
    for fault, probability in fault_prediction.items():
        # Check if the fault is in the reaction_thresholds dictionary
        if fault in reaction_thresholds:
            # Compare the probability of the fault with its threshold
            if probability > reaction_thresholds[fault]:
                # If the probability exceeds the threshold, add the fault to the reaction list
                fault_reaction.append(fault)
...
def plan_route(current, goal, allowed, sandy, game_size, sandy_flag=True):
    ...
    ### GRAD STUDENTS REPLACE BREADTH FIRST WITH BEST FIRST OR ASTAR ### #### FOXY PA2
...
def hybrid_agent(environment,grid,start_x,start_y,sandy):
    # Grad students will need to augment this agent with a way to track the movement actions taken in sandy vs. non-sandy areas. ###FOXY GRAB THIS FROM PA2
    sim_length = 1000
    ...
    while sim_time < sim_length:
        ...
        if len(plan) == 0:
            ...
            #storm_probs = compute_storm_probs(cell_bayes_nets,ppm_evidence,sandyground_evidence,facts) # storm_probs is a 2D array matching the grid
            storm_probs = compute_storm_probs(cell_bayes_nets,ppm_evidence,sandyground_evidence,facts,grid) # storm_probs is a 2D array matching the grid
            not_unsafe = list()
            for cell in cell_facts:
                cell_dict = dict(cell)
        ...
        #print("Current time: "+str(sim_time))
    ...
    return 1, total_movement_cost, sim_time, safe_points
...
for i in range(sims):
    ...
    result, sandy_mvmt_cost, elapsed_sim_time = hybrid_agent(sandy_env,grid,start_x,start_y,True)
