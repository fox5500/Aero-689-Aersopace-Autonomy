...
class BayesNetCategorical:
    ...
    def add(self, node_spec):
        ...
        self.nodes.append(node)
        self.variables.append(node.variable)
        self.var_vals.append(node.variable_values)
        for parent in node.parents:
            self.variable_node(parent).children.append(node)
...
class BayesNodeCategorical:
    ...
    def __init__(self, X, parents, cpt, variable_values):
        ...
        * A dict {(v1, v2, ...): p, ...}, the distribution P(X=true |
        ...
        # We store the table always in the third form above.
        if isinstance(cpt, (float, int, list)):  # no parents, 0-tuple
            cpt = {(): cpt}
...
def elimination_ask(X, e, bn):
    ...
    for var in reversed(bn.variables):
        #print("Calling make_factor with var:", var, "and evidence:", e) ##DEBUG
...
def make_factor(var, e, bn):
    ...
    """
    #print("In make_factor, var:", var, "evidence:", e)
...
#    variables = [X for X in [var] + node.parents if X not in e]
#
#    # Debugging: Print the cpt structure
#    print("CPT structure for var:", var, "is:", node.cpt)
#
#    cpt = {}
#    for e1 in all_events(variables, bn, e):
#        try:
#            cpt_value = node.p(e1[var], e1)
#            cpt[event_values(e1, variables)] = cpt_value
#            print(f"Event: {e1}, Event for var: {e1[var]}, Event values: {event_values(e1, variables)}, Node probability: {cpt_value}")
#        except KeyError as ke:
#            print(f"KeyError encountered. Key: {ke}, Event: {e1}, Event for var: {e1[var]}")
...
class PlanRouteSandy(Problem):
    ...
    def path_cost(self, cost, state, action, next_state):
        ###
        #if action == "forward" or action == "rotate_cw" or action == "rotate_ccw" and self.cell in self.sandy_cells:
        #    return c + 2
        #else:
        #    return c + 1
        ### Undergrads can comment this function out (or delete the whole class.) ### FOXY PA2
        path_cost = 0
        x, y = state.get_location()
...
def greedy_best_first_search(problem, heuristic):
    ...
    if problem.goal_test(node.state):
        return node
    frontier = PriorityQueue(order='min', f=lambda node: heuristic(node))
    frontier.append(node)
    explored = set()
    while frontier:
        node = frontier.pop()
        ...
        if problem.goal_test(node.state):
            return node
        for child in node.expand(problem):
            if child.state not in explored and not frontier_contains_state(frontier, child.state):
                frontier.append(child)
    return None

def frontier_contains_state(frontier, state):
    return state in [node.state for _, node in frontier.heap]
