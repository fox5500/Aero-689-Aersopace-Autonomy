...
#    # Print the bayesnets
#    for cell, bayesnet in bayesnets.items():
#        print(f"Bayes Net for cell {cell}:")
...
def get_ppm_evidence(facts):
    ...
    ppm_evidence_dict = {}
    for fact in facts:
        ## Print the current fact for debugging
        #count =+ 1
        #print("Current fact:", count, fact)
        fact_dict = dict(fact)
        x = fact_dict["xloc"]
        y = fact_dict["yloc"]
        ppm = fact_dict["air_quality"]
        if ppm != "unknown":
            ppm_evidence_dict[(x,y)] = ppm
    print("PPM Evidence Dictionary:", ppm_evidence_dict)
    return ppm_evidence_dict
...
def get_sandyground_evidence(facts):
    ...
    for fact in facts:
        ...
        if sandy != "unknown":
            ...
            if sandy == "false":
                sandy = False
            sandy_evidence_dict[(x,y)] = sandy
    print("sand Evidence Dictionary:", sandy_evidence_dict)
    return sandy_evidence_dict

def compute_storm_probs(bayes_nets, ppm_evidence, sandyground_evidence, safe_cells):
    # Convert safe_cells to a list for easier checking
    safe_cell_list = []
    for safe_cell in safe_cells:
        safe_cell_list.append((safe_cell[0], safe_cell[1]))  # Changed to x, y format
    ...
    for x in range(6):
        for y in range(6):
            # Check if the cell is a safe cell
            if (x, y) in safe_cell_list:
                prob_grid[x][y] = 0  # Set probability to 0 for safe cells
            else:
                # Gather evidence for this cell
                evidence = {}
                if (x, y) in ppm_evidence:
                    evidence['air_quality'] = ppm_evidence[(x, y)]
                if (x, y) in sandyground_evidence:
                    evidence['sandy'] = sandyground_evidence[(x, y)]  # Added sandy evidence

                # Query the Bayesian network for the storm probability
                bayes_net = bayes_nets[(x, y)]
                storm_prob = elimination_ask('Storm', evidence, bayes_net).prob(True)
                prob_grid[x][y] = storm_prob  # Store the computed storm probability
                print("Coordinates:", (x, y), "Evidence provided:", evidence)
